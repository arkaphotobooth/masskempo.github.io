 ...
 * Version 10.0 (Ultimate Auto-Matchmaking, Live Visual Bracket & True Grand Final)
 */
...
function handleCSVUpload(event) { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = function(e) { const rows = e.target.result.split('\n'); let count = 0, errors = 0; rows.forEach((row, i) => { if(i === 0 || !row.trim()) return; const cols = row.split(',').map(item => item.trim().replace(/^"|"$/g, '')); if(cols.length >= 3) { const nama = cols[0], kontingen = cols[1], kategori = cols[2]; if(nama && STATE.categories.some(c => c.name.toLowerCase() === kategori.toLowerCase())) { STATE.participants.push({ id: Date.now() + i, nama, kontingen, kategori, urut: 0, pool: '-', isFinalist: false, urutFinal: 0, losses: 0, scores: { b1: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 }, b2: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 } }, finalScore: 0, techScore: 0 }); count++; } else errors++; } }); saveToLocalStorage(); refreshAllData(); event.target.value = ''; alert(`${count} Atlet diimport.`); }; reader.readAsText(file); }
...
// ============================================================================
// --- TEMPLATE BLUEPRINT & LIVE VISUAL BRACKET ---
// ============================================================================

// Blueprint Double Elim 8. Kolom 'col' digunakan untuk Swimlane CSS.
const TEMPLATE_8 = [
    { matchNum: 1, babak: "WB Q-Final", col: 1, slot1: 1, slot2: 2, nextW: 5, nextL: 7 },
    { matchNum: 2, babak: "WB Q-Final", col: 1, slot1: 3, slot2: 4, nextW: 5, nextL: 8 },
    { matchNum: 3, babak: "WB Q-Final", col: 1, slot1: 5, slot2: 6, nextW: 6, nextL: 8 },
    { matchNum: 4, babak: "WB Q-Final", col: 1, slot1: 7, slot2: 8, nextW: 6, nextL: 7 },
    
    { matchNum: 5, babak: "WB S-Final", col: 2, slot1: null, slot2: null, nextW: 11, nextL: 10 },
    { matchNum: 6, babak: "WB S-Final", col: 2, slot1: null, slot2: null, nextW: 11, nextL: 9 },
    
    { matchNum: 7, babak: "LB R1", col: 1, slot1: null, slot2: null, nextW: 9, nextL: null },
    { matchNum: 8, babak: "LB R1", col: 1, slot1: null, slot2: null, nextW: 10, nextL: null },
    
    { matchNum: 9, babak: "LB Q-Final", col: 2, slot1: null, slot2: null, nextW: 12, nextL: null },
    { matchNum: 10, babak: "LB Q-Final", col: 2, slot1: null, slot2: null, nextW: 12, nextL: null },
    
    { matchNum: 11, babak: "WB Final", col: 3, slot1: null, slot2: null, nextW: 'WINNER', nextL: 13 },
    { matchNum: 12, babak: "LB S-Final", col: 3, slot1: null, slot2: null, nextW: 13, nextL: null },
    { matchNum: 13, babak: "LB Final", col: 4, slot1: null, slot2: null, nextW: 'RUNNER-UP', nextL: null }
];
...
function generateRandoriBracket() {
    ...
    if(existingMatches.length > 0) {
        if(!confirm("Bagan sudah ada! Mengacak ulang akan menghapus semua data pertandingan di bagan ini. Yakin?")) return;
        STATE.matches = STATE.matches.filter(m => m.kategori !== catName);
    }

    if(count > 8) return alert("Bagan Otomatis saat ini mendukung maksimal 8 peserta per Pool. (Template 16 akan datang).");
    
    const templateSize = 8;
...
// MENGGAMBAR BAGAN VISUAL CSS (LIVE UPDATE)
function renderVisualBracket(catName) {
    ...
    // Kelompokkan berdasarkan kolom (swimlane)
    ...
    columns.forEach(colNum => {
        ...
        if(colNum < 4) {
            colHTML += `<div class="flex flex-col justify-center"><div class="w-8 border-b-2 border-slate-600"></div></div>`;
        }
...
function checkExistingDrawing() {
    ...
    panelEmbu.classList.add('hidden'); panelRandori.classList.add('hidden'); panelEmpty.classList.add('hidden'); resultDiv.innerHTML = '';
    ...
    if(categoryObj && categoryObj.discipline === 'randori') {
        ...
        renderVisualBracket(catName); // Render the Live Visual Bracket
    } else {
        // Logika Embu manual (tetap)
        panelEmbu.classList.remove('hidden');
        const isFinalMode = list.some(p => p.isFinalist); 
        if (isFinalMode) { let finalL = list.filter(p => p.isFinalist); if (finalL.some(p => p.urutFinal > 0)) { finalL.sort((a,b) => a.urutFinal - b.urutFinal); renderPoolUI(finalL, "POOL FINAL (Sudah Diundi)", resultDiv, true); } else { resultDiv.innerHTML = `<div class="col-span-1 md:col-span-2 text-center text-yellow-500 py-10 border-2 border-dashed border-yellow-600 rounded-xl">Peserta Final telah dipilih. Klik Acak Urutan.</div>`; } } else if (list.some(p => p.urut > 0)) { list.sort((a,b) => a.urut - b.urut); if(list.some(p => p.pool === 'A' || p.pool === 'B')) { renderPoolUI(list.filter(p => p.pool === 'A'), "POOL A", resultDiv, false); renderPoolUI(list.filter(p => p.pool === 'B'), "POOL B", resultDiv, false); } else { renderPoolUI(list, "BABAK PENYISIHAN", resultDiv, false); } } else { resultDiv.innerHTML = `<div class="col-span-1 md:col-span-2 text-center text-slate-500 py-10 border-2 border-dashed border-slate-700 rounded-xl">Kategori ini belum diundi.</div>`; }
    }
}
...
// --- TAB 4: SCORING (TRUE GRAND FINAL LOGIC) ---
function filterPesertaScoring() {
    ...
    if(categoryObj.discipline === 'randori') {
        ...
        if(catMatches.length === 0) {
            selectEl.innerHTML = `<option value="">-- Tidak ada Partai Aktif --</option>`; document.getElementById('scoring-athlete-name').innerText = "-"; return;
        }
        selectEl.innerHTML = catMatches.sort((a,b)=>a.matchNum - b.matchNum).map((m) => {
            ...
            return `<option value="match-${m.id}">Partai ${m.matchNum} [${m.babak}] ${mrh.nama} vs ${pth.nama}</option>`;
        }).join('');
...
function saveRandoriMatchResult() {
    ...
    if(confirm(`Konfirmasi Pemenang: ${winnerName}\nSkor: ${sMerah} - ${sPutih}\n\nSistem akan mencatat hasil ke bagan visual. Lanjutkan?`)) {
        ...
        // ==== LOGIKA TRUE GRAND FINAL (RESET MATCH) ====
        // Jika ini adalah partai final, dan yg menang adalah yg punya 1 loss (LB Winner)
        if(match.nextW === 'WINNER' && winnerP.losses === 1) {
            ...
            // Buat match extra (F7) secara dinamis
            let extraMatch = {
                id: Date.now(), kategori: match.kategori, matchNum: match.matchNum + 1,
                babak: "TRUE GRAND FINAL", col: 4, nextW: 'WINNER', nextL: null,
                merahId: match.merahId, putihId: match.putihId, // Posisi pita dipertahankan
                winnerId: null, status: 'pending', skorMerah: 0, skorPutih: 0
            };
            STATE.matches.push(extraMatch);
        } 
        // Lanjutkan lemparan normal jika bukan True Final scenario
        else {
...
// --- TAB 5, 6, 7: RANKING, JUARA, ADMIN (MINIFIED, LOGIKA TETAP) ---

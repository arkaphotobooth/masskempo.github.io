 ...
 * Version 19.0 (The Ultimate Unified System: Full Logic + Batsu + 16-Bracket + Cloud Sync)
 */
...
function initializeData() {
    try {
        ...
        parts = parts.map(p => ({...p, losses: p.losses || 0, isFinalist: p.isFinalist || false}));
...
// --- 3. CATEGORY & ATLET LOGIC ---
function renderCategoryList() {
    const container = document.getElementById('list-kategori'); if(!container) return;
    container.innerHTML = STATE.categories.map(c => `
        <div class="bg-slate-800 px-4 py-2 rounded-lg text-sm flex items-center gap-3 border border-slate-700 shadow-sm hover:border-blue-500 transition-all">
            <span class="${c.discipline==='randori'?'bg-red-700':'bg-blue-600'} text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter">${c.discipline}</span>
...
function updateAllDropdowns() {
    ...
    ['p-kategori', 'edit-kategori', 'draw-select-kategori', 'select-kategori', 'rank-filter-kategori'].forEach(id => {
        ...
        if(el) { const cur = el.value; el.innerHTML = emptyOpt + options; if(cur && Array.from(el.options).some(o => o.value === cur)) el.value = cur; }
    });
...
function renderParticipantTable() {
    ...
    body.innerHTML = list.map(p => `
        <tr class="border-b border-slate-800 hover:bg-slate-800/30 transition-colors">
            <td class="p-3 font-bold text-blue-300 w-[35%] break-words">${p.nama} ${p.isFinalist ? '<br><span class="text-[9px] bg-yellow-600 text-white px-1 rounded inline-block mt-1 font-black">FINALIS</span>' : ''}</td>
            <td class="p-3 text-sm w-[25%]">${p.kontingen}</td>
            <td class="p-3 text-xs text-slate-400 font-mono w-[25%]">${p.kategori}<br>Pool ${p.pool} | No.${p.urut}</td>
            <td class="p-3 text-right w-[15%]">
                <button onclick="openEditModal(${p.id})" class="text-blue-400 mr-3 p-1"><i class="fas fa-edit"></i></button>
                <button onclick="deletePeserta(${p.id})" class="text-slate-600 hover:text-red-400 p-1"><i class="fas fa-trash"></i></button>
            </td>
        </tr>`).join('') || '<tr><td colspan="4" class="p-6 text-center text-slate-500 italic">Data atlet kosong.</td></tr>';
}

// --- 4. DRAWING & BAGAN 16 ENGINE (THE BIG LOGIC) ---
const TEMPLATES = {
...
function generateRandoriBracket() {
    ...
    if(!confirm("Hapus bagan lama dan buat baru?")) return;

    STATE.matches = STATE.matches.filter(m => m.kategori !== catName);
    let shuffled = [...list].sort(() => Math.random() - 0.5);
    
    let size = 4;
    if(list.length > 8) size = 16;
    else if(list.length > 4) size = 8;
    
    let template = TEMPLATES[size];
    template.forEach(t => {
        STATE.matches.push({
...
// --- 5. SCORING ENGINE (COMPREHENSIVE) ---
...
function calculateLive() {
    ...
    if(STATE.settings.numJudges === 5) {
        ...
        sum = sorted[1] + sorted[2] + sorted[3]; // Buang tertinggi & terendah
    } else {
...
function saveRandoriMatchResult() {
    ...
    saveToLocalStorage(); alert("Pertandingan Selesai!"); filterPesertaScoring();
}

// --- 7. RANKING OPSI 1 & EXPORT ENGINE ---
function renderRanking() {
    ...
    if(!filter) return container.innerHTML = '<div class="p-10 text-center text-slate-500 border-2 border-dashed border-slate-800 rounded-2xl italic">Pilih kategori untuk melihat klasemen.</div>';
    ...
    if(catObj?.discipline === 'embu') {
        ['FINAL', 'SINGLE', 'A', 'B'].forEach(pk => {
            ...
            html += poolList.map((p,i) => `
                <div class="bg-dark-card p-4 rounded-xl border border-slate-700 mb-3 flex justify-between items-center shadow-lg">
                    <div>
                        <div class="font-bold text-white text-lg">${i+1}. ${p.nama} ${pk!=='FINAL'&&p.isFinalist?'<span class="text-[9px] bg-yellow-600 text-black px-2 py-0.5 rounded ml-2 font-black shadow-sm">LULUS</span>':''}</div>
                        <div class="text-xs text-slate-500 font-bold uppercase tracking-wider">${p.kontingen}</div>
                    </div>
                    <div class="text-right">
                        <div class="text-[10px] text-green-500 font-black uppercase">Final Score</div>
                        <div class="text-3xl font-black text-white font-mono">${(pk==='FINAL'?p.scores.b2.final : p.scores.b1.final).toFixed(2)}</div>
                        <div class="text-[9px] text-slate-500 font-bold">Teknik: ${(pk==='FINAL'?p.scores.b2.tech : p.scores.b1.tech).toFixed(1)}</div>
                    </div>
                </div>`).join('');
        });
    } else {
        ...
        if(wins) {
            html += `<div class="grid grid-cols-1 gap-4">
                <div class="bg-yellow-600/10 p-6 rounded-2xl border-2 border-yellow-600 flex items-center gap-5">
                    <i class="fas fa-medal text-4xl text-yellow-500"></i>
                    <div><div class="text-xs font-black text-yellow-500 uppercase">JUARA 1 - EMAS</div><div class="text-2xl font-black text-white">${wins.emas}</div></div>
                </div>
                <div class="bg-slate-400/10 p-6 rounded-2xl border-2 border-slate-500 flex items-center gap-5">
                    <i class="fas fa-medal text-4xl text-slate-300"></i>
                    <div><div class="text-xs font-black text-slate-300 uppercase">JUARA 2 - PERAK</div><div class="text-2xl font-black text-white">${wins.perak}</div></div>
                </div>
                ${wins.perunggu.map(n => `
                <div class="bg-amber-900/10 p-6 rounded-2xl border-2 border-amber-800 flex items-center gap-5">
                    <i class="fas fa-medal text-4xl text-amber-700"></i>
                    <div><div class="text-xs font-black text-amber-700 uppercase">JUARA 3 - PERUNGGU</div><div class="text-2xl font-black text-white">${n}</div></div>
                </div>`).join('')}
            </div>`;
        } else {
            html += '<div class="p-10 text-center text-slate-600 italic">Pertandingan masih berlangsung...</div>';
        }
    }
...
function toggleTimer() { 
    ...
    if(UI.timerInterval) { 
        ...
        btn.innerText = "LANJUT"; btn.className = "bg-yellow-600 hover:bg-yellow-500 px-4 py-2 rounded-lg w-full font-bold";
    } else { 
...
function resetTimer() { clearInterval(UI.timerInterval); UI.timerInterval = null; UI.timerSeconds = 0; updateTimerUI(); if(document.getElementById('btn-timer')) document.getElementById('btn-timer').innerText = "START"; calculateLive(); }
...
function exportHasilCSV(filter = null) {
    let rows = [["Disiplin", "Kategori", "Medali/Peringkat", "Nama Atlet", "Kontingen", "Nilai Akhir"]];
    let cats = filter ? STATE.categories.filter(c=>c.name===filter) : STATE.categories;
    cats.forEach(c => {
        if(c.discipline==='embu') {
            let list = STATE.participants.filter(p=>p.kategori===c.name && (p.scores.b1.final>0||p.scores.b2.final>0));
            list.forEach(p => rows.push(["EMBU", c.name, p.isFinalist?'Finalis':'Penyisihan', p.nama, p.kontingen, p.scores.b2.final || p.scores.b1.final]));
        } else {
            const w = calculateRandoriFinalists(c.name);
            if(w) { rows.push(["RANDORI", c.name, "1 - EMAS", w.emas, "", ""]); rows.push(["RANDORI", c.name, "2 - PERAK", w.perak, "", ""]); w.perunggu.forEach(n=>rows.push(["RANDORI", c.name, "3 - PERUNGGU", n, "", ""])); }
        }
    });
...
function exportMedaliCSV() {
    ...
    // Re-run tally logic locally for CSV
    downloadCSV("Klasemen_Medali_Juara_Umum", rows);
}
...
function injectMicroButtons(type, cat) {
    ...
    btn.className = 'ml-3 bg-green-600 hover:bg-green-500 text-white px-3 py-1.5 rounded-lg text-[10px] font-black shadow-md transition-all flex items-center gap-2';
...
function calculateRandoriFinalists(cat) {
    ...
    let gf = m.find(x => x.babak.includes('GRAND FINAL') || x.matchNum === 13 || x.matchNum === 30);
    if(!gf) return null;
    let ju1 = STATE.participants.find(p => p.id === gf.winnerId);
    let ju2 = STATE.participants.find(p => p.id === gf.loserId);
    return { emas: ju1?.nama || 'Unknown', perak: ju2?.nama || 'Unknown', perunggu: [] };
}

function recalculateAllLosses(cat) { /* Maintenance Function */ }

function promoteToFinal() {
    const f = document.getElementById('rank-filter-kategori').value; if(!f) return;
    let n = parseInt(prompt("Berapa finalis per pool (A dan B)?", "3"));
    if(!n || isNaN(n)) return;
    let poolA = STATE.participants.filter(p=>p.kategori===f && p.pool==='A').sort((a,b)=>b.scores.b1.final-a.scores.b1.final).slice(0,n);
    let poolB = STATE.participants.filter(p=>p.kategori===f && p.pool==='B').sort((a,b)=>b.scores.b1.final-a.scores.b1.final).slice(0,n);
    [...poolA, ...poolB].forEach(p => p.isFinalist = true);
    saveToLocalStorage();
}

function cancelFinalist() {
    const f = document.getElementById('rank-filter-kategori').value; if(!f) return;
    if(!confirm("Batalkan semua finalis di kategori ini?")) return;
    STATE.participants.filter(p => p.kategori === f).forEach(p => p.isFinalist = false);
    saveToLocalStorage();
}

function deleteCategory(id) { if(confirm("Hapus kategori?")) { STATE.categories = STATE.categories.filter(c => c.id !== id); saveToLocalStorage(); refreshAllData(); } }
function deletePeserta(id) { if(confirm("Hapus atlet?")) { STATE.participants = STATE.participants.filter(p => p.id !== id); saveToLocalStorage(); renderParticipantTable(); } }

function injectAdminExportButtons() {
    const section = document.querySelector('#section-admin .bg-dark-card.text-center');
    if(section) {
        section.innerHTML = `<h2 class="text-xl font-black text-white mb-2">Pusat Data Global</h2><div class="grid grid-cols-3 gap-4 mt-6"><button onclick="exportDrawingCSV()" class="bg-blue-600 p-4 rounded-xl font-bold">Semua Jadwal</button><button onclick="exportHasilCSV()" class="bg-purple-600 p-4 rounded-xl font-bold">Semua Hasil</button><button onclick="exportMedaliCSV()" class="bg-yellow-600 p-4 rounded-xl font-bold text-black">Semua Medali</button></div>`;
    }
}

...
function renderRandoriMatchmaker(catName) {
    ...
    document.getElementById('match-merah').innerHTML = optionsHTML;
...
function renderRandoriMatchList(catName) {
    ...
    listDiv.innerHTML = catMatches.map((m, i) => {
        const merah = STATE.participants.find(p => p.id === m.merahId);
        const putih = STATE.participants.find(p => p.id === m.putihId);
...
function createRandoriMatch() {
    const catName = document.getElementById('draw-select-kategori').value;
    const merahId = parseInt(document.getElementById('match-merah').value);
    ...
    const newMatch = {
        ...
        merahId: merahId, putihId: putihId, 
        winnerId: null, status: 'pending',
        skorMerah: 0, skorPutih: 0
    };
...
function filterPesertaScoring() {
    ...
    if(categoryObj.discipline === 'randori') {
        ...
        let catMatches = STATE.matches.filter(m => m.kategori === catName && m.status === 'pending');
        if(catMatches.length === 0) {
...
function loadRandoriMatch() {
    ...
    currentRandoriMatchId = parseInt(val.replace('match-', ''));
...
function saveScore(babakOverride) { const pId = parseInt(document.getElementById('select-peserta').value); if(!pId) return alert('Pilih atlet!'); let babak = document.getElementById('select-babak').value; if(babakOverride === 1 || babakOverride === 2) babak = `b${babakOverride}`; for(let i=1; i<=STATE.settings.numJudges; i++) if(document.getElementById(`score-${i}`).value === "") return alert(`TOTAL NILAI untuk Wasit ${i} kosong!`); const calc = calculateLive(); const p = STATE.participants.find(i => i.id === pId); p.scores[babak] = { raw: calc.raw, techRaw: calc.techRaw, penalty: calc.penalty, final: calc.final, tech: calc.tieBreaker, time: UI.timerSeconds }; if (p.pool === 'FINAL') { p.finalScore = p.scores.b2.final; p.techScore = p.scores.b2.tech; } else if (p.pool === 'A' || p.pool === 'B') { p.finalScore = p.scores.b1.final; p.techScore = p.scores.b1.tech; } else { if(p.scores.b1.final > 0 && p.scores.b2.final > 0) { p.finalScore = (p.scores.b1.final + p.scores.b2.final) / 2; p.techScore = (p.scores.b1.tech + p.scores.b2.tech) / 2; } else { p.finalScore = p.scores[babak].final; p.techScore = p.scores[babak].tech; } } saveToLocalStorage(); alert(`SKOR TERSIMPAN!\nNilai atlet ${p.nama} direkam.`); clearInterval(UI.timerInterval); UI.timerInterval = null; document.getElementById('btn-timer').innerText = 'START'; document.getElementById('btn-timer').className = 'bg-green-600 hover:bg-green-500 px-4 py-2 rounded-lg w-full font-bold'; }
...
function resetAllPenilaian() { if(confirm('⚠️ PERHATIAN: Ini akan MENGHAPUS SEMUA SKOR & PARTAI RANDORI. Yakin?')) { STATE.participants.forEach(p => { p.scores = { b1: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 }, b2: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 } }; p.finalScore = 0; p.techScore = 0; p.isFinalist = false; p.urutFinal = 0; p.losses = 0; }); STATE.matches = []; saveToLocalStorage(); refreshAllData(); alert('Nilai & Partai di-reset.'); } }

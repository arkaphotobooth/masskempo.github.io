...
async function initFirebase() {
    ...
    try {
        ...
        if (badge) badge.innerHTML = '<span style="color: #ef4444">○ OFFLINE</span>';
    }
}
...
function handleCSVUpload(event) { 
    ...
    reader.onload = function(e) { 
        ...
        refreshAllData(); 
        event.target.value = ''; 
        alert(`${count} Tim/Atlet diimport sukses.`); 
    }; 
...
document.getElementById('form-edit-peserta').addEventListener('submit', (e) => { 
    ...
    if (idx > -1) { 
        if (STATE.participants[idx].kategori !== newKategori) { 
            ...
            STATE.participants[idx].isFinalist = false; 
            STATE.participants[idx].losses = 0; 
            STATE.participants[idx].scores = { 
                b1: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time:0 }, 
                b2: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time:0 } 
            }; 
...
function generateRandoriBracket() {
    ...
    try {
        ...
        saveToLocalStorage();
        // Ini yang akan mengirim bagan ke semua laptop
...
function undoMatchResult(matchId) {
    ...
    if (!loserId) { 
        loserId = (match.winnerId === match.merahId) ? match.putihId : match.merahId;
    }
        ...
        STATE.matches.filter(m => m.kategori === catName && m.status === 'pending').forEach(match => {
            if (match.merahId !== null && match.putihId !== null) {
                if (match.merahId === -1 || match.putihId === -1) {
                    ...
                    if (match.merahId === -1 && match.putihId === -1) { 
                        match.winnerId = -1; match.loserId = -1; 
                    } else { 
                        match.winnerId = match.merahId === -1 ? match.putihId : match.merahId; match.loserId = -1; 
                    }
...
function applyDrawingData(arr, poolName) { 
    arr.forEach((p, index) => { 
        ...
        if (found) { found.urut = index + 1; found.pool = poolName; }
    });
}
...
function saveRandoriMatchResult() {
    ...
    if (confirm(`Simpan hasil G-${match.matchNum % 50 || 50}?`)) {
        ...
        STATE.matches[matchIdx].status = 'done';
        
        // Ini yang bikin partai hilang dari dropdown
        // 2. MAJUKAN PEMENANG
        ...
        // Ini akan membersihkan dropdown karena status sudah 'done'
        // Sembunyikan panel
...
function updateTimerUI() { 
    document.getElementById('timer-display').innerText = `${Math.floor(UI.timerSeconds / 60).toString().padStart(2, '0')}:${(UI.timerSeconds % 60).toString().padStart(2, '0')}`;
}
...
function cancelFinalist() {
    ...
    catParts.forEach(p => {
        if (p.isFinalist) {
            ...
            if (p.pool === 'FINAL') {
                ...
                if (takenA && !takenB) p.pool = 'B'; 
                else if (takenB && !takenA) p.pool = 'A'; 
                else p.pool = 'A'; 
            }
            changed = true;
        }
    ...
    if (changed) { 
        saveToLocalStorage(); 
        alert("Status Finalis dibatalkan!"); 
        renderRanking(); 
        checkExistingDrawing(); 
        filterPesertaScoring(); 
    }
...
function promoteToFinal() {
    ...
    if (confirm("Tetapkan " + combined.length + " peserta ini sebagai Finalis?")) {
        combined.forEach(w => { 
            let p = STATE.participants.find(x => x.id === w.id); 
            if (p) { p.isFinalist = true; p.urutFinal = 0; } 
        });
        saveToLocalStorage(); 
        alert("Finalis ditetapkan!"); 
        renderRanking(); 
        checkExistingDrawing(); 
        filterPesertaScoring();
    }
...
function renderRanking() { 
    ...
    if (!hasData) {
        if (catObj.discipline === 'randori') { 
            return container.innerHTML = `<div class="p-10 text-center text-slate-500 border border-dashed border-slate-700 rounded-xl">Turnamen Randori belum selesai / belum ada medali.</div>`;
        } else { 
...
function renderJuaraUmum() { 
    ...
    let leaderboard = Object.keys(tally).map(kontingen => ({ 
        nama: kontingen, 
        emas: tally[kontingen].g, 
        perak: tally[kontingen].s, 
        perunggu: tally[kontingen].b, 
            ...
            if (finalis.length > 0) {
                ...
                ['SINGLE', 'A', 'B'].forEach(poolKey => {
                    ...
                    poolList.forEach((p, i) => { 
                        rows.push(["EMBU", cat.name, `Pool ${poolKey} Rank ${i+1}`, p.nama, p.kontingen, p.scores.b1.final.toFixed(2)]); 
                    });
                });
            }
        } else {
...
function resetAllPenilaian() { 
    if (confirm('⚠️ PERHATIAN: Ini akan MENGHAPUS SEMUA SKOR & PARTAI RANDORI. Yakin?')) { 
...
function resetDataAtlet() { 
    if (confirm('⚠️ PERHATIAN: Ini MENGHAPUS SEMUA ATLET. Yakin?')) { 
        STATE.participants = [];
        STATE.matches = []; 
        saveToLocalStorage(); 
        refreshAllData(); 
        alert('Data atlet dihapus.'); 
    } 
}

function resetTotalSistem() { 
    if (confirm('🚨 FACTORY RESET: Hapus seluruh sistem?')) { 
        localStorage.clear();
        STATE = { categories: [], participants: [], matches: [], settings: { numJudges: 5 } }; 
        refreshAllData();
        alert('Sistem kembali ke pengaturan awal.'); 
        location.reload(); 
    } 
}

 ...
 * Version 7.1 (Randori Scoreboard Logic & Toggle Warnings)
 */
...
// State sementara khusus Papan Skor Randori
let RANDORI_STATE = {
    merah: { score: 0, warn1: false, warn2: false },
    putih: { score: 0, warn1: false, warn2: false }
};
...
document.getElementById('form-kategori').addEventListener('submit', (e) => { 
    ...
    STATE.categories.push({ id: Date.now(), name, type, discipline }); 
...
function renderCategoryList() { 
    ...
    container.innerHTML = STATE.categories.map(c => {
        let badgeColor = c.discipline === 'randori' ? 'bg-red-700' : 'bg-blue-600';
        let disciplineText = c.discipline ? c.discipline.toUpperCase() : 'EMBU';
        return `<div class="bg-slate-800 px-4 py-2 rounded-lg text-sm flex items-center gap-3 border border-slate-700 shadow-sm"><span class="${badgeColor} text-[9px] px-1.5 py-0.5 rounded font-bold">${disciplineText}</span><span class="font-bold text-white">${c.name}</span><span class="bg-slate-700 text-[10px] px-2 py-0.5 rounded text-slate-300">${c.type} Org</span><button onclick="deleteCategory(${c.id})" class="text-slate-500 hover:text-red-400 ml-2"><i class="fas fa-times"></i></button></div>`
    }).join(''); 
}
...
// --- TAB 4: SCORING (ADAPTIVE UI & RANDORI BOARD) ---
function filterPesertaScoring() {
    ...
    if(categoryObj && categoryObj.discipline === 'randori') {
        ...
        resetRandoriBoard(); // Reset UI saat berpindah
    } else {
...
function updateScoringButtonsUI() {
    ...
    if(!pId || !selectBabak || !btnB1) return;
    ...
    if(isFinalMode && p.isFinalist) selectBabak.innerHTML = `<option value="b2">Babak Final</option>`;
    else if(p.pool === 'A' || p.pool === 'B') selectBabak.innerHTML = `<option value="b1">Babak Penyisihan</option>`;
    else selectBabak.innerHTML = `<option value="b1">Babak 1</option><option value="b2">Babak 2</option>`;
...
function updateRandoriUI() {
    ...
    // Toggle visual lingkaran Peringatan Merah
    ...
    // Toggle visual lingkaran Peringatan Putih
    ...
    document.getElementById('warn2-putih').className = RANDORI_STATE.putih.warn2 ? "w-6 h-6 rounded-full transition-colors bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.8)]" : "w-6 h-6 rounded-full border-2 border-orange-500 transition-colors bg-transparent";
}
    ...
    for(let i=1; i<=n; i++) {
        container.innerHTML += `
            <div class="bg-slate-900 p-3 rounded-lg border border-slate-600 focus-within:border-blue-500 transition-colors">
                <div class="text-center mb-2 pb-2 border-b border-slate-700"><label class="block text-[10px] text-slate-400 uppercase font-bold">Wasit ${i}</label></div>
                <div class="space-y-2">
                    <div><label class="block text-[9px] text-slate-500 mb-1">TOTAL NILAI</label><input type="number" step="0.5" id="score-${i}" oninput="calculateLive()" class="w-full bg-slate-800 p-2 rounded text-2xl font-black outline-none text-center text-white placeholder-slate-700" placeholder="0"></div>
                    <div><label class="block text-[9px] text-slate-500 mb-1 flex justify-between"><span>TEKNIK</span> ${i===1?'<span class="text-yellow-500 font-bold">TIE-BREAK</span>':''}</label><input type="number" step="0.5" id="tech-${i}" oninput="calculateLive()" class="w-full bg-slate-800 p-2 rounded text-sm font-bold outline-none text-center ${i===1?'text-yellow-400':'text-blue-300'} placeholder-slate-700" placeholder="Opsional"></div>
                </div>
            </div>`;
    }
    calculateLive();
...
function loadExistingScores() {
    const pId = parseInt(document.getElementById('select-peserta').value);
    const babak = document.getElementById('select-babak').value;
    if(!pId || !babak) return;
    ...
    if(scoreData && scoreData.raw && scoreData.raw.length > 0) {
        ...
        for(let i=1; i<=STATE.settings.numJudges; i++) {
            document.getElementById(`score-${i}`).value = ''; document.getElementById(`tech-${i}`).value = '';
        }
...
function saveScore(babakOverride) {
    ...
    for(let i=1; i<=STATE.settings.numJudges; i++) if(document.getElementById(`score-${i}`).value === "") return alert(`TOTAL NILAI untuk Wasit ${i} kosong!`);
    ...
    saveToLocalStorage(); alert(`SKOR TERSIMPAN!\nNilai atlet ${p.nama} direkam.`);
    clearInterval(UI.timerInterval); UI.timerInterval = null; document.getElementById('btn-timer').innerText = 'START'; document.getElementById('btn-timer').className = 'bg-green-600 hover:bg-green-500 px-4 py-2 rounded-lg w-full font-bold';
}
...
// --- TAB 5, 6, 7 (MINIFIED, LOGIKA TETAP SAMA) ---
function promoteToFinal() { const filterCat = document.getElementById('rank-filter-kategori').value; if(filterCat === 'all' || !filterCat) return alert("Pilih satu kategori spesifik!"); let list = STATE.participants.filter(p => p.kategori === filterCat && p.scores.b1.final > 0); let poolA = list.filter(p => p.pool === 'A').sort((a,b) => b.scores.b1.final - a.scores.b1.final || b.scores.b1.tech - a.scores.b1.tech).slice(0, 3); let poolB = list.filter(p => p.pool === 'B').sort((a,b) => b.scores.b1.final - a.scores.b1.final || b.scores.b1.tech - a.scores.b1.tech).slice(0, 3); if(poolA.length === 0 && poolB.length === 0) return alert("Belum ada data penyisihan."); if(confirm(`Promosikan ${poolA.length} dari Pool A dan ${poolB.length} dari Pool B ke Final?`)) { let finalParticipants = [...poolA, ...poolB]; finalParticipants.forEach(p => { const idx = STATE.participants.findIndex(x => x.id === p.id); if(idx > -1) { STATE.participants[idx].isFinalist = true; STATE.participants[idx].urutFinal = 0; } }); saveToLocalStorage(); alert("Babak Final dibuat!"); switchTab('drawing'); } }

 ...
 * Version 6.0 (Edit Scores & Optional Technical Tie-Breaker)
 */
...
function switchTab(targetTab) {
    UI.tabs.forEach(tab => {
        ...
        if (tabEl) { 
            ...
            if(tab === 'admin') tabEl.classList.add('text-red-400'); else if(tab === 'juara') tabEl.classList.add('text-yellow-500'); else tabEl.classList.add('text-slate-400'); 
        }
    });
...
// --- TAB KATEGORI, ATLET, DRAWING (LOGIKA TETAP) ---
document.getElementById('form-kategori').addEventListener('submit', (e) => { e.preventDefault(); const name = document.getElementById('cat-name').value.trim(); const type = parseInt(document.getElementById('cat-type').value); if(!name) return; if(STATE.categories.some(c => c.name.toLowerCase() === name.toLowerCase())) return alert("Kategori sudah ada!"); STATE.categories.push({ id: Date.now(), name, type }); saveToLocalStorage(); refreshAllData(); e.target.reset(); });
function renderCategoryList() { const container = document.getElementById('list-kategori'); if(STATE.categories.length === 0) return container.innerHTML = `<span class="text-sm text-slate-500 italic">Belum ada kategori.</span>`; container.innerHTML = STATE.categories.map(c => `<div class="bg-slate-800 px-4 py-2 rounded-lg text-sm flex items-center gap-3 border border-slate-700 shadow-sm"><span class="font-bold text-blue-300">${c.name}</span><span class="bg-slate-700 text-[10px] px-2 py-0.5 rounded text-slate-300">${c.type} Org</span><button onclick="deleteCategory(${c.id})" class="text-slate-500 hover:text-red-400 ml-2"><i class="fas fa-times"></i></button></div>`).join(''); }
...
function handleCSVUpload(event) { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = function(e) { const rows = e.target.result.split('\n'); let count = 0, errors = 0; rows.forEach((row, i) => { if(i === 0 || !row.trim()) return; const cols = row.split(',').map(item => item.trim().replace(/^"|"$/g, '')); if(cols.length >= 3) { const nama = cols[0], kontingen = cols[1], kategori = cols[2]; if(nama && STATE.categories.some(c => c.name.toLowerCase() === kategori.toLowerCase())) { STATE.participants.push({ id: Date.now() + i, nama, kontingen, kategori, urut: 0, pool: '-', isFinalist: false, urutFinal: 0, scores: { b1: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 }, b2: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 } }, finalScore: 0, techScore: 0 }); count++; } else errors++; } }); saveToLocalStorage(); refreshAllData(); event.target.value = ''; alert(`${count} Atlet diimport.\n(${errors} gagal/kategori tidak valid).`); }; reader.readAsText(file); }
document.getElementById('form-peserta').addEventListener('submit', (e) => { e.preventDefault(); const catName = document.getElementById('p-kategori').value; if(!catName) return alert("Pilih kategori!"); STATE.participants.push({ id: Date.now(), nama: document.getElementById('p-nama').value, kontingen: document.getElementById('p-kontingen').value, kategori: catName, urut: 0, pool: '-', isFinalist: false, urutFinal: 0, scores: { b1: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 }, b2: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 } }, finalScore: 0, techScore: 0 }); saveToLocalStorage(); renderParticipantTable(); document.getElementById('p-nama').value = ''; document.getElementById('p-nama').focus(); });
function renderParticipantTable() { const body = document.getElementById('table-peserta-body'); const filter = document.getElementById('filter-atlet-kategori').value; let list = filter && filter !== 'all' ? STATE.participants.filter(p => p.kategori === filter) : STATE.participants; if(list.length === 0) return body.innerHTML = `<tr><td colspan="5" class="p-6 text-center text-slate-500">Tidak ada data atlet.</td></tr>`; let sortedList = [...list].sort((a,b) => a.kategori === b.kategori ? a.urut - b.urut : a.kategori.localeCompare(b.kategori)); body.innerHTML = sortedList.map(p => `<tr class="border-b border-slate-800 hover:bg-slate-800/50 transition-colors"><td class="p-4 font-bold text-blue-300">${p.nama} ${p.isFinalist ? '<span class="text-[10px] bg-yellow-500 text-black px-1 rounded ml-1">FINALIS</span>' : ''}</td><td class="p-4">${p.kontingen}</td><td class="p-4 text-xs text-slate-400">${p.kategori}</td><td class="p-4 text-center">${p.urut > 0 ? `<span class="bg-slate-700 px-2 py-1 rounded text-xs font-mono">No.${p.urut} | Pool ${p.pool}</span>` : `<span class="text-xs text-red-400 italic">Belum Undian</span>`}</td><td class="p-4 text-right"><button onclick="openEditModal(${p.id})" class="text-blue-400 hover:text-blue-300 transition-colors mr-3"><i class="fas fa-edit"></i></button><button onclick="deletePeserta(${p.id})" class="text-slate-500 hover:text-red-500 transition-colors"><i class="fas fa-trash"></i></button></td></tr>`).join(''); }
...
document.getElementById('form-edit-peserta').addEventListener('submit', (e) => { e.preventDefault(); const id = parseInt(document.getElementById('edit-id').value); const newKategori = document.getElementById('edit-kategori').value; const idx = STATE.participants.findIndex(p => p.id === id); if(idx > -1) { if(STATE.participants[idx].kategori !== newKategori) { STATE.participants[idx].urut = 0; STATE.participants[idx].pool = '-'; STATE.participants[idx].isFinalist = false; STATE.participants[idx].urutFinal = 0; STATE.participants[idx].scores = { b1: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time:0 }, b2: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time:0 } }; STATE.participants[idx].finalScore = 0; STATE.participants[idx].techScore = 0; } STATE.participants[idx].nama = document.getElementById('edit-nama').value; STATE.participants[idx].kontingen = document.getElementById('edit-kontingen').value; STATE.participants[idx].kategori = newKategori; saveToLocalStorage(); renderParticipantTable(); closeEditModal(); alert("Data diperbarui."); } });
function checkExistingDrawing() { const catName = document.getElementById('draw-select-kategori').value; const resultDiv = document.getElementById('drawing-result'); resultDiv.innerHTML = ''; if(!catName) return; let list = STATE.participants.filter(p => p.kategori === catName); const isFinalMode = list.some(p => p.isFinalist); if (isFinalMode) { let finalL = list.filter(p => p.isFinalist); if (finalL.some(p => p.urutFinal > 0)) { finalL.sort((a,b) => a.urutFinal - b.urutFinal); renderPoolUI(finalL, "POOL FINAL (Sudah Diundi)", resultDiv, true); } else { resultDiv.innerHTML = `<div class="col-span-1 md:col-span-2 text-center text-yellow-500 py-10 border-2 border-dashed border-yellow-600 rounded-xl">Peserta Final telah dipilih. Klik "Acak Urutan Sekarang" untuk Drawing Final.</div>`; } } else if (list.some(p => p.urut > 0)) { list.sort((a,b) => a.urut - b.urut); if(list.some(p => p.pool === 'A' || p.pool === 'B')) { renderPoolUI(list.filter(p => p.pool === 'A'), "POOL A (Sudah Diundi)", resultDiv, false); renderPoolUI(list.filter(p => p.pool === 'B'), "POOL B (Sudah Diundi)", resultDiv, false); } else { renderPoolUI(list, "BABAK PENYISIHAN (Sudah Diundi)", resultDiv, false); } } else resultDiv.innerHTML = `<div class="col-span-1 md:col-span-2 text-center text-slate-500 py-10 border-2 border-dashed border-slate-700 rounded-xl">Kategori ini belum diundi.</div>`; }
function startDrawing() { const catName = document.getElementById('draw-select-kategori').value; if(!catName) return alert("Pilih kategori!"); let list = STATE.participants.filter(p => p.kategori === catName); if(list.length === 0) return alert("Belum ada peserta!"); const resultDiv = document.getElementById('drawing-result'); resultDiv.innerHTML = ''; const isFinalMode = list.some(p => p.isFinalist); if (isFinalMode) { let finalL = list.filter(p => p.isFinalist); if (finalL.some(p => p.urutFinal > 0)) if (!confirm("⚠️ Finalis SUDAH DIUNDI.\nYakin ingin mengacak ulang urutan Final?")) return; shuffleArray(finalL); finalL.forEach((p, index) => { const idx = STATE.participants.findIndex(x => x.id === p.id); STATE.participants[idx].urutFinal = index + 1; }); renderPoolUI(finalL, "POOL FINAL", resultDiv, true); } else { if (list.some(p => p.urut > 0)) { if (!confirm("⚠️ Kategori ini SUDAH DIUNDI.\nYakin ingin mengacak ulang?")) return; list.forEach(p => { p.scores = { b1: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time:0 }, b2: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time:0 } }; p.finalScore = 0; p.techScore = 0; }); } shuffleArray(list); if (list.length > 6) { const half = Math.ceil(list.length / 2); const poolA = list.slice(0, half); const poolB = list.slice(half); applyDrawingData(poolA, 'A'); applyDrawingData(poolB, 'B'); renderPoolUI(poolA, "POOL A", resultDiv, false); renderPoolUI(poolB, "POOL B", resultDiv, false); } else { applyDrawingData(list, 'SINGLE'); renderPoolUI(list, "BABAK PENYISIHAN", resultDiv, false); } } saveToLocalStorage(); renderParticipantTable(); }
...
// --- TAB 4: SCORING & EDIT SYSTEM BARU ---
...
// LOGIKA BARU: Mengatur opsi dropdown babak saat atlet dipilih
function updateScoringButtonsUI() {
    const pId = parseInt(document.getElementById('select-peserta').value);
    const selectBabak = document.getElementById('select-babak');
    if(!pId || !selectBabak) return;
    
    const p = STATE.participants.find(i => i.id === pId);
    
    // Ganti isi dropdown berdasarkan status Pool Atlet
    selectBabak.innerHTML = '';
    const isFinalMode = STATE.participants.some(x => x.kategori === p.kategori && x.isFinalist);

    if(isFinalMode && p.isFinalist) {
        selectBabak.innerHTML = `<option value="b2">Babak Final</option>`;
    } else if(p.pool === 'A' || p.pool === 'B') {
        selectBabak.innerHTML = `<option value="b1">Babak Penyisihan</option>`;
    } else {
        selectBabak.innerHTML = `<option value="b1">Babak 1</option><option value="b2">Babak 2</option>`;
    }
    
    // Auto-load nilai yang sudah pernah disimpan (jika ada)
    loadExistingScores();
}

// LOGIKA BARU: Menampilkan 2 Input per Juri (Total & Teknik)
    ...
    for(let i=1; i<=n; i++) {
        container.innerHTML += `
            <div class="bg-slate-900 p-3 rounded-lg border border-slate-600 focus-within:border-blue-500 transition-colors">
                ...
                <div class="space-y-2">
                    <div>
                        <label class="block text-[9px] text-slate-500 mb-1">TOTAL NILAI (WAJIB)</label>
                        <input type="number" step="0.5" id="score-${i}" oninput="calculateLive()" class="w-full bg-slate-800 p-2 rounded text-2xl font-black outline-none text-center text-white placeholder-slate-700" placeholder="0">
                    </div>
                    <div>
                        <label class="block text-[9px] text-slate-500 mb-1 flex justify-between">
                            <span>NILAI TEKNIK</span> ${i===1?'<span class="text-yellow-500 font-bold">TIE-BREAK</span>':''}
                        </label>
                        <input type="number" step="0.5" id="tech-${i}" oninput="calculateLive()" class="w-full bg-slate-800 p-2 rounded text-sm font-bold outline-none text-center ${i===1?'text-yellow-400':'text-blue-300'} placeholder-slate-700" placeholder="Opsional">
                    </div>
                </div>
            </div>`;
    }
...
// LOGIKA BARU: Mengambil data dari localStorage dan memasukkannya kembali ke input
function loadExistingScores() {
    const pId = parseInt(document.getElementById('select-peserta').value);
    const babak = document.getElementById('select-babak').value; // 'b1' atau 'b2'
    if(!pId || !babak) return;
    ...
    if(scoreData && scoreData.raw && scoreData.raw.length > 0) {
        // Jika sebelumnya disimpan dgn 3 juri, pastikan UI berubah ke 3 juri
        const nJudges = scoreData.raw.length;
        if(STATE.settings.numJudges !== nJudges) setJudges(nJudges);

        for(let i=1; i<=nJudges; i++) {
            document.getElementById(`score-${i}`).value = scoreData.raw[i-1] || '';
            // Load nilai teknik jika pernah diisi
            document.getElementById(`tech-${i}`).value = (scoreData.techRaw && scoreData.techRaw[i-1]) ? scoreData.techRaw[i-1] : '';
        }
        // Load timer waktu sebelumnya
        UI.timerSeconds = scoreData.time || 0;
        updateTimerUI();
    } else {
        // Jika data kosong, bersihkan semua input
        for(let i=1; i<=STATE.settings.numJudges; i++) {
            document.getElementById(`score-${i}`).value = '';
            document.getElementById(`tech-${i}`).value = '';
        }
        UI.timerSeconds = 0;
        updateTimerUI();
    }
...
function calculateLive() {
    ...
    for(let i=1; i<=STATE.settings.numJudges; i++) {
        ...
        techRaw.push(parseFloat(document.getElementById(`tech-${i}`).value) || 0); // Ambil nilai teknik opsional
    }
    ...
    // Nilai teknik wasit 1 sebagai tie-breaker
    return { final, penalty, raw, techRaw, tieBreaker: techRaw[0] };
}

// LOGIKA BARU: Simpan Nilai Lebih Sederhana
function saveScore() {
    const pId = parseInt(document.getElementById('select-peserta').value); 
    ...
    // Validasi hanya Total Nilai yang wajib diisi
    for(let i=1; i<=STATE.settings.numJudges; i++) {
        if(document.getElementById(`score-${i}`).value === "") return alert(`TOTAL NILAI untuk Wasit ${i} tidak boleh kosong!`);
    }

    const calc = calculateLive(); 
    const p = STATE.participants.find(i => i.id === pId);

    // Simpan ke database JSON lokal
    p.scores[babak] = { 
        raw: calc.raw, 
        techRaw: calc.techRaw, 
        penalty: calc.penalty, 
        final: calc.final, 
        tech: calc.tieBreaker, // Tie Breaker = Nilai Teknik Wasit 1
        time: UI.timerSeconds
    };

    // Sinkronisasi Nilai Akhir untuk Ranking
    if (p.pool === 'FINAL') {
        ...
        // Jika Single, hitung rata-rata jika B1 & B2 terisi
    ...
    saveToLocalStorage(); 
    alert(`SKOR TERSIMPAN!\nNilai atlet ${p.nama} berhasil direkam ke sistem.`);
    
    // Timer direset untuk persiapan atlet berikutnya, namun nilai dipertahankan di UI sbg konfirmasi
...
// --- TAB 5, 6, 7: RANKING, JUARA, ADMIN (TETAP SAMA) ---
function promoteToFinal() {
    ...
    if(confirm(`Anda akan mempromosikan ${poolA.length} atlet dari Pool A dan ${poolB.length} atlet dari Pool B ke Babak Final.\nLanjutkan?`)) {
        ...
        finalParticipants.forEach(p => {
            ...
            if(idx > -1) { STATE.participants[idx].isFinalist = true; STATE.participants[idx].urutFinal = 0; }
        });
        saveToLocalStorage();
...
function renderRanking() {
    const filter = document.getElementById('rank-filter-kategori').value; let list = STATE.participants;
    ...
    if (filter !== 'all') {
        ...
        const hasPools = catParticipants.some(p => p.pool === 'A' || p.pool === 'B'); const hasFinal = catParticipants.some(p => p.isFinalist);
        if(hasPools && !hasFinal) btnPromote.classList.remove('hidden'); else btnPromote.classList.add('hidden');
        list = catParticipants;
    } else btnPromote.classList.add('hidden');

    const container = document.getElementById('ranking-list'); let hasData = list.some(p => p.scores.b1.final > 0 || p.scores.b2.final > 0);
    ...
    ['FINAL', 'SINGLE', 'A', 'B'].forEach(poolKey => {
        ...
        htmlOutput += poolList.map((p, i) => {
            ...
            let displayScoreHTML = ''; let displayFinalHTML = '';
            
            if (poolKey === 'FINAL') { displayScoreHTML = `<div class="text-[10px] text-slate-500 uppercase">Skor Final (B2)</div><div class="text-sm font-mono text-slate-300">${p.scores.b2.final.toFixed(1)}</div>`; displayFinalHTML = `<div class="text-2xl font-black text-white">${p.scores.b2.final.toFixed(2)}</div>`; } 
            else if (poolKey === 'SINGLE') { displayScoreHTML = `<div class="text-[10px] text-slate-500 uppercase">B1 / B2</div><div class="text-sm font-mono text-slate-300">${p.scores.b1.final.toFixed(1)} / ${p.scores.b2.final.toFixed(1)}</div>`; displayFinalHTML = `<div class="text-2xl font-black text-white">${p.finalScore.toFixed(2)}</div>`; } 
            else { displayScoreHTML = `<div class="text-[10px] text-slate-500 uppercase">Skor Penyisihan</div><div class="text-sm font-mono text-slate-300">${p.scores.b1.final.toFixed(1)}</div>`; displayFinalHTML = `<div class="text-2xl font-black text-white">${p.scores.b1.final.toFixed(2)}</div>`; }
...
function exportCustomCSV() {
    ...
    list.forEach(p => { rows.push([`"${p.nama}"`, `"${p.kontingen}"`, `"${p.kategori}"`, p.pool, p.isFinalist ? "Ya" : "Tidak", p.scores.b1.final, p.scores.b2.final, p.scores.b1.tech, p.scores.b2.tech]); });
...
function renderJuaraUmum() {
    ...
    STATE.categories.forEach(cat => {
        ...
        const hasFinal = list.some(p => p.isFinalist); let winners = [];
        if(hasFinal) { winners = list.filter(p => p.isFinalist && p.scores.b2.final > 0).sort((a,b) => b.scores.b2.final - a.scores.b2.final || b.scores.b2.tech - a.scores.b2.tech); } 
        else { winners = list.filter(p => p.pool === 'SINGLE' && p.scores.b1.final > 0).sort((a,b) => b.finalScore - a.finalScore || b.techScore - a.techScore); }
    ...
    if(leaderboard.length === 0) return tbody.innerHTML = `<tr><td colspan="6" class="p-8 text-center text-slate-500 border-b border-slate-700">Belum ada data medali.</td></tr>`;
...
function resetAllPenilaian() { if(confirm('⚠️ PERHATIAN: Ini akan MENGHAPUS SEMUA SKOR. Yakin?')) { STATE.participants.forEach(p => { p.scores = { b1: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 }, b2: { raw: [], techRaw: [], penalty: 0, final: 0, tech: 0, time: 0 } }; p.finalScore = 0; p.techScore = 0; p.isFinalist = false; p.urutFinal = 0; }); saveToLocalStorage(); refreshAllData(); alert('Nilai di-reset.'); } }

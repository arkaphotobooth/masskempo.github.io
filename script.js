 ...
 * Version 7.0 (Fundamental Adaptive UI: Embu vs Randori)
 */
...
function initializeData() {
    try {
        ...
        // Auto-migrate struktur data lama agar mendukung disiplin
...
// --- TAB 1: KATEGORI (UPDATE DISIPLIN) ---
document.getElementById('form-kategori').addEventListener('submit', (e) => { 
    ...
    const discipline = document.getElementById('cat-discipline').value; // Ambil nilai Embu/Randori
...
function renderCategoryList() { 
    ...
    container.innerHTML = STATE.categories.map(c => {
        // Beri warna badge berbeda untuk Embu vs Randori
        ...
        <div class="bg-slate-800 px-4 py-2 rounded-lg text-sm flex items-center gap-3 border border-slate-700 shadow-sm">
            <span class="${badgeColor} text-[9px] px-1.5 py-0.5 rounded font-bold">${disciplineText}</span>
...
// --- TAB 2 & 3: ATLET & DRAWING (MINIFIED KARENA TIDAK ADA PERUBAHAN LOGIKA) ---
...
function updateScoringButtonsUI() {
    ...
    // Sembunyikan semua tombol simpan embu (Diganti tombol tunggal nanti, tapi biarkan logika tombol multi ini sbg fallback)
...
function saveScore(babakOverride) {
    ...
    // Karena kita memakai 1 tombol umum, kita ambil babak mana yang sedang aktif di layar
    let babak = document.getElementById('select-babak').value; 
    
    // (Fallback logika lama jika diklik dari tombol spesifik)
...
// --- TAB 5, 6, 7: RANKING, JUARA, ADMIN (MINIFIED, LOGIKA TETAP SAMA) ---
